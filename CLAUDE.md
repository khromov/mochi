...
- `packages/admin` — admin-panel `create-mochi` template source (login, dashboard, products CRUD, profile) built on Svelte + Tailwind v4. Port 3338. Like `minimal`/`demos` it doubles as a published template, so it may only depend on published packages — never `mochi-shared`. Its stateful concerns (sessions, password hashing, validation, rate limiting, migrations) ship as `src/lib/*` stubs over seed data.
...
bun run dev          # scripts/dev.ts: auto-discovers every package with a `dev` script and runs them
                     # in parallel with color-prefixed logs — site (3333), demos (3334), minimal (3335), support (3336), admin (3338)
