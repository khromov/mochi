...
- `packages/shared` — `mochi-shared`: tiny private helpers shared by the deployed sites (currently just the Umami `analytics` handle). **Only for `site` / `demos` / `support` — never import it from `packages/mochi`**, which is published to npm and must not depend on a private workspace package. See its README.
