...
- `packages/msgpackr-extract-stub` — published as `@mochi-framework/msgpackr-extract-stub`; a pure-JS stub wired in via the root `overrides` so the native `msgpackr-extract` is never built. Retained after the bunqueue→queuert migration removed the only consumer of `msgpackr`: the override is now a harmless no-op, kept in place so the package stays in the publish/deploy flow.
