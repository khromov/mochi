...
- `packages/docker` — `mochi-docker`: internal docker-compose stack for local dev — Postgres 15 (5432, `postgres`/`postgres`) + Adminer (8080), data bind-mounted at `./.db` (gitignored, no named volume). Auto-started by `bun run dev`; its dev script warns and idles when Docker is unavailable (so it never triggers dev.ts's first-exit teardown). `bun run db:stop` / `db:down` / `db:reset` from the root.
