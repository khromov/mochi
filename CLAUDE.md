...
**A `Mochi.serve()` `outDir` must live inside the project tree** — the existing tests `mkdtempSync` into `packages/mochi/`, not `/tmp` or the session scratchpad. Mind the depth: a test at `src/*.test.ts` uses `path.join(import.meta.dir, '..', '.mochi-…-')`, while one in a subfolder (`src/runtime/`, `src/compiler/`, …) needs `'..', '..'`. Getting this wrong doesn't fail — it silently writes build output into `src/`, which is gitignored and only surfaces later as bizarre `svelte-check` errors in a directory nobody wrote. The on-demand server-island path compiles each island to `<outDir>/svelte-compile/<Name>-<hash>.server.js` and dynamically `import()`s it; that module's deps (framework runtime, `@noble/ciphers`) resolve relative to `outDir`, so an `outDir` under `/tmp` has no `node_modules` chain back to the project and fails with `Cannot find module '…/svelte-compile/….server.js' from ''` (a 500 at `/_mochi/island/:name`). Only the on-demand path is affected — the prebuilt/manifest path bakes absolute `ssrModule` paths at build time. When writing an ad-hoc `Mochi.serve()` repro, put `outDir` under the package even though generic temp files belong in the scratchpad.

#### The Bun EISDIR bundler bug and the hoisted-linker requirement

The repo's historical "EISDIR" workarounds traced back to one Bun 1.3.14 bug, empirically bisected 2026-07-19: inside a `bun test` process, a **second** `Bun.build()` fails with `EISDIR reading file` on any node_modules file that (a) the test process's module loader already loaded and (b) appears in that build's module graph — but **only under Bun's isolated install linker**, where node_modules resolves through the symlinked `node_modules/.bun` store. All three conditions are necessary: plain `bun run` never fails, a first build never fails, unloaded or out-of-graph deps never fail, and the hoisted layout never fails. Victims rotate per run among whatever sits in that intersection (`@noble/ciphers`, `@joint-ops/hitlimit-bun`, `ipaddr.js`); the dep named in old comments was always incidental. Root `bunfig.toml` therefore pins `[install] linker = "hoisted"` — do not remove it, and don't reintroduce subprocess-build indirection in tests: calling `build()` in-process from a test is fine under the hoisted layout (see `buildServerIslandManifest.test.ts`). If EISDIR ever reappears, first check that `node_modules/.bun` doesn't exist (a stale isolated install survives linker changes until a clean reinstall). Pinning the hoisted linker removes only this double-build reason for per-file isolation — it does **not** let us drop `run-tests.ts`. `Mochi.serve()` still pins a one-per-process singleton (`__mochi_config__`, plus siblings `__mochi_image_runtime__`/captcha/image/email config) that `server.stop()` never clears, so plain `bun test` works for pure unit tests but any second server-booting file throws "Mochi.serve() has already been called." — full-app tests stay one-file-per-process.
...
- To branch SSR-only fallback logic on whether the current render will hydrate, call `isHydratable()` (imported from `mochi-framework`, during component init like `getContext`) — it works at any nesting depth inside an island subtree, and is a constant `true` in client bundles. The signal travels from the call site to the island root as the internal `__mochi_hydratable` transport prop (injected by the preprocessor / also-hydrate envelope / client bootstrap), which a compile-pass prologue (`injectHydratableContextSeed`) turns into Svelte context; there is no public `isHydratable` prop.
