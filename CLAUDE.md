...
- **Vendoring a _fork_** (`packages/mochi/src/vendor/devalue/`) inverts the rule below, deliberately: the npm dep **stays**, upstream stays verbatim `.js` under `<dir>/src/` behind one hand-written typed `index.ts`, and the folder is `.prettierignore`d. The copy is a published release _plus unreleased commits_, so it has to stay diffable against upstream and provably byte-identical to the release — `packages/mochi/src/devalueParity.test.ts` asserts both and fails loudly if `node_modules/devalue` has been linked to a working tree. Mark a fork `<version>+<sha>` in the `// Vendored from` comment, and never re-vendor it from `npm pack`.
