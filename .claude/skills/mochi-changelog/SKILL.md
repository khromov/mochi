...
4. Write `CHANGELOG-NEW.md` as a **release announcement** (this gets posted to Discord/GitHub). Use **simple, portable Markdown that renders identically on GitHub and Discord** — only `#`/`##` headings, `-` bullets, and `**bold**`. Format:
   ...
   **Fixes & misc**
   ...
   - **Add a `#`-anchor when the item is a sub-section of a shared page.** A whole-page link (`/docs/<slug>`) is right when the feature *is* the page; when several items map to the same page, or the feature is one heading within a broader page, deep-link to that heading instead. To find the anchor:
