...
| `remotion`, `@remotion/*`, `react`, `react-dom` (`packages/remotion`)       | Remotion's renderer, its CLI and React must move together on one exact version; a partial bump breaks `remotion studio`/`render`.                                                                                                                                 |
