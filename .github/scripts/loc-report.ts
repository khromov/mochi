...
const PACKAGES: Package[] = [
  {
    ...
    categories: [
      ...
      'src/__fixtures__/**',
      'src/Mochi.ts',
      'src/compiler/ComponentRegistry.ts',
      'src/compiler/**',
      'src/runtime/hooks.ts',
      'src/runtime/{requestContext,forms,formsJson,errors}.ts',
      'src/runtime/cookies*.ts',
      'src/runtime/enhance*.ts',
      'src/runtime/{csrf,proxy}.ts',
      'src/runtime/**',
      'src/islands/serverIslandCrypto.ts',
      'src/islands/**',
      'src/events.ts',
      'src/extensions.ts',
      'src/dev/consoleLogger.ts',
      'src/dev/**',
      'src/cache/**',
      'src/sql/**',
      'src/tasks/**',
      ...
      'src/debug-bar/**',
      'src/templates/**',
      // Explicit filenames, not a `src/*.ts` wildcard, so a new top-level file
      // still surfaces in "Other" instead of being silently absorbed.
      'src/{index,highlight,mochiConfig,lifecycle}.ts',
    ],
  },
  {
    name: 'packages/docs',
  ...
  },
  {
    ...
    categories: [
      ...
      'src/og/**',
      'src/leak-test/**',
      'src/{Site,Docs,Blog,BlogPost}.svelte',
      'src/{index,routes,demoIndex}.ts',
      'src/{shell.html,*.d.ts}',
    ],
  },
  {
    ...
    categories: ['src/hn/**', 'src/admin/**', 'src/todo/**', 'src/lib/**', 'src/Landing.svelte', 'src/index.ts', 'src/{shell.html,*.d.ts}'],
  },
  {
  ...
  },
  {
    ...
    categories: ['src/**/*.test.ts', 'src/cli*', 'src/{create,templates,utils}.ts', 'src/index.ts'],
  },
  {
    name: 'packages/video-animations',
    root: join(REPO_ROOT, 'packages', 'video-animations'),
    categories: ['src/{frame,anim,theme}.ts', 'src/{render,render-worker,generate}.ts', 'src/{prepare-fonts,fonts-dir,probe}.ts', 'src/*.d.ts'],
  },
  {
    name: 'packages/msgpackr-extract-stub',
    root: join(REPO_ROOT, 'packages', 'msgpackr-extract-stub'),
    categories: ['*.test.ts', 'index.{ts,js,cjs}'],
    scanGlob: '*.{ts,js,cjs}',
  },
];
