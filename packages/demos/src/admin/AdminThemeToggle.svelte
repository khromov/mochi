...
<button type="button" class="theme-toggle" data-theme-state={theme} aria-label={LABEL[theme]} title={LABEL[theme]} onclick={toggle}>
  <span class="icon" aria-hidden="true">
    {#if !mounted}
      <span class="icon-placeholder"></span>
    {:else if theme === 'auto'}
      <SunMoon size={15} strokeWidth={1.6} />
    {:else if theme === 'dark'}
...
<style>
  ...
  .theme-toggle:focus-visible {
    outline: none;
    box-shadow: 0 0 0 3px color-mix(in srgb, var(--admin-accent) 28%, transparent);
  }
