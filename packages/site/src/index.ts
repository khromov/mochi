...
await Mochi.serve({
  ...
  handle: sequence(compress(), immutableAssets, helloWorld, agentDiscoveryLinks, asciiDog, analytics, encodeDebugBarPaths, noCache, cookieVaryTestHandle, shotHandle),
  handleError,
  idleTimeout: 60,
  ...
  filters: {
    ...
    // /mcp and the extensionless /.well-known/api-catalog must answer at exactly those paths;
    // the site-wide trailingSlash: 'always' policy would otherwise 308 them to a slash form
    // that breaks discovery (and which some MCP clients don't follow).
