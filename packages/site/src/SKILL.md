...
1. **Discover.** Fetch the index: `https://mochi.fast/llms.txt`. It lists every doc under `## Docs`, every demo under `## Examples`, and every blog post under `## Blog` — each a one-line description plus a link to that section's own `llms.txt`.
...
- `/llms.json` is a machine-readable variant of the index — `{ title, description, url }` per doc, post, and demo — if structured selection is easier than parsing the text index.
- The changelog is served as the `changelog` doc entry (`/docs/changelog/llms.txt`) — reach for it to answer "what changed in version X" questions.
