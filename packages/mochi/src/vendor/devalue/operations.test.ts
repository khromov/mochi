// Ported verbatim from the vendored devalue fork's own test suite (upstream test/operations.test.js); only the imports
...
import { assert, suite as uvuSuite } from './uvuShim';
import { stringify, stringifyAsync, defaultStringifyOperations, filterArrayIndices, parse } from './index';
...
suite('operations option', (test) => {
  ...
  test('defaultStringifyOperations and shapeOf sentinels are frozen', () => {
    ...
    assert.ok(Object.isFrozen(defaultStringifyOperations.shapeOf({ [Symbol('key')]: 1 })));
  });
  ...
  test('identify controls deduplication and cycle detection', () => {
    ...
    const shapeOf = (value) => (value instanceof Wrapper ? defaultStringifyOperations.shapeOf(value.inner) : defaultStringifyOperations.shapeOf(value));
...
suite('side-effect-free operations', (test) => {
  ...
  test('get override reads through descriptors without invoking getters', () => {
    ...
    assert.throws(() => stringify(thing, undefined, { operations: { get } }), /refusing to invoke getter for "computed"/);
    assert.equal(getter_calls, 0);
...
suite('handle-based operations', (test) => {
  ...
  function assert_parity(value) {
    assert.equal(stringify(h(value), undefined, { operations: handle_operations }), stringify(value));
  }
  ...
  test('reducers receive the handle, not the raw value', () => {
    ...
    const result = stringify(
      h(new Custom('yes')),
      {
        Custom: (handle) => (handle instanceof Handle && raw(handle) instanceof Custom ? h(raw(handle).inner) : false),
      },
  ...
  test('async: thenables resolve through toPromise', async () => {
    ...
    const [expected, actual] = await Promise.all([stringifyAsync(value), stringifyAsync(h(value), undefined, { operations: handle_operations })]);
...
function tripwire(value) {
  ...
  for (const trap of TRIPWIRE_TRAPS) {
    ...
    handler[trap] = (_target, ...args) => {
      ...
      const detail = typeof args[0] === 'string' || typeof args[0] === 'symbol' ? ` (${String(args[0])})` : '';
...
suite('tripwire operations (value is never touched)', (test) => {
  ...
  test('reducers receive the untouched proxy', () => {
    ...
    const result = stringify(
      ...
      {
        Custom: (value) => untrip(value) instanceof Custom && tripwire({ inner: untrip(value).inner }),
      },
      { operations: tripwire_operations },
    );
...
suite('filterArrayIndices', (test) => {
  ...
  test('matches the default operation for the same value', () => {
    const array = [1, 2, 3];
    array.extra = 'x';
    assert.equal(filterArrayIndices(Object.keys(array)), defaultStringifyOperations.indicesOf(array));
  });
});
