// Ported verbatim from the vendored devalue fork's own test suite (upstream test/index.test.js); only the imports
...
import { assert, suite, test } from './uvuShim';
import * as vm from 'node:vm';
import * as consts from './src/constants.js';
import { uneval, unflatten, parse, stringify, stringifyAsync } from './index';
...
for (const [name, tests] of Object.entries(fixtures)) {
  const test = suite(`stringify: ${name}`);
  for (const t of tests) {
    test(t.name, () => {
...
for (const [name, tests] of Object.entries(fixtures)) {
  const test = suite(`unflatten: ${name}`);
  for (const t of tests) {
    test(t.name, () => {
...
const invalid = [
  ...
  },
  {
    name: 'empty string',
    json: '',
    // Upstream asserts V8's wording; these two cases are the host JSON parser's message, not devalue's, and
    // JavaScriptCore words them differently.
...
for (const fn of [uneval, stringify]) {
  ...
  test(`${fn.name} populates error.keys and error.path`, () => {
    try {
      fn({
        foo: {
  ...
  test(`${fn.name} populates error.value with the problematic value`, () => {
  ...
  test(`${fn.name} populates error.root with the root value`, () => {
    const root = {
      foo: {
        array: [function invalid() {}],
      },
  ...
  test(`${fn.name} includes value and root on arbitrary non-POJOs error`, () => {
    class Whatever {}
  ...
  test(`${fn.name} includes value and root on symbolic keys error`, () => {
...
for (const [name, tests] of Object.entries(fixtures)) {
  const test = suite(`stringifyAsync: ${name}`);
  for (const t of tests) {
    test(t.name, async () => {
...
for (const [name, tests] of Object.entries(fixtures)) {
  const test = suite(`stringifyAsync round-trip: ${name}`);
  for (const t of tests) {
    test(t.name, async () => {
...
const asyncTests = suite('stringifyAsync: promises');
...
const asyncErrorTests = suite('stringifyAsync: errors');
...
const circularCustomTypes = suite('circular references through custom types');
...
{
  const test = suite('uneval: large graphs');
