// Ported verbatim from the vendored devalue fork's own test suite (upstream test/parse-operations.test.js); only the imports
...
import { stringify, parse, unflatten, defaultParseOperations, defaultStringifyOperations } from './index';
...
suite('cross-realm operations', (test) => {
  ...
  function realm_operations(context) {
    ...
    return {
      ...
      fromViewInfo: (tag, buffer, byteOffset, length) => {
        ...
        return byteOffset !== undefined ? new Constructor(buffer, byteOffset, length) : new Constructor(buffer);
      },
  ...
  test('typed arrays are constructed in the target realm', () => {
    ...
    context.probe = revived;
    assert.ok(vm.runInContext('probe instanceof Uint8Array && probe.buffer instanceof ArrayBuffer', context));
  });
  ...
  test('cyclic values are linked correctly across realms', () => {
    ...
    context.probe = revived;
    assert.ok(vm.runInContext('probe.self === probe && probe.list[0] === probe', context));
  });
});
...
suite('tripwire parse operations', (test) => {
  ...
  const operations = {
    ...
    fromViewInfo: (tag, buffer, byteOffset, length) => tripwire(defaultParseOperations.fromViewInfo(tag, untrip(buffer), byteOffset, length)),
...
const handle_operations = {
  ...
  fromViewInfo: (tag, buffer, byteOffset, length) => h(defaultParseOperations.fromViewInfo(tag, raw(buffer), byteOffset, length)),
