...
export function unflatten(parsed, revivers, options) {
	...
	// as in `stringify`: when nothing is overridden the construction operations
	// are known to be the plain host-JavaScript ones, so the hot paths below
	// inline them rather than paying for a call per value
	const native = ops === default_parse_operations;
	...
	const has_revivers = !!revivers;

	// With the default operations and no revivers, nothing can revive to
	// `undefined`, so a slot holding `undefined` is unambiguously "not yet
	// hydrated" and the `in` operator can be skipped entirely.
	const simple = native && !has_revivers;
	...
	function hydrate(index) {
		// the overwhelmingly common case is a plain, in-range index; everything
		// else — the negative sentinels, non-numbers, the standalone call —
		// is handled off the fast path
		...
		const cached = hydrated[index];
		if (cached !== undefined || (!simple && index in hydrated)) return cached;

		// `values` grows when a builtin-serialized payload is appended for a
		// custom reviver, so this has to read the live length
		...
		if (!value || typeof value !== 'object') {
			return (hydrated[index] = native ? value : ops.fromPrimitive(value));
		} else if (Array.isArray(value)) {
			if (typeof value[0] === 'string') {
				const type = value[0];

				const reviver = has_revivers && Object.hasOwn(revivers, type) ? revivers[type] : undefined;
				...
				switch (type) {
					case 'Date':
						hydrated[index] = native ? new Date(value[1]) : ops.fromISOString(value[1]);
						break;

					case 'Set': {
						const set = native ? new Set() : ops.createSet();
						hydrated[index] = set;
					...
					case 'Map': {
						const map = native ? new Map() : ops.createMap();
						hydrated[index] = map;
					...
					case 'RegExp':
						hydrated[index] = native
							? new RegExp(value[1], value[2])
							: ops.fromRegExpInfo(value[1], value[2]);
						break;
					...
					case 'null': {
						const obj = native ? Object.create(null) : ops.createNullPrototypeObject();
						hydrated[index] = obj;
						for (let i = 1; i < value.length; i += 2) {
							const key = value[i];
				...
				const array = native ? new Array(n) : ops.createArray(n);
				hydrated[index] = array;
				...
				for (let i = 0; i < n; i += 1) {
					...
					if (native) {
						array[i] = hydrate(child);
					} else {
						ops.set(array, i, hydrate(child));
					}
				}
			...
			const object = native ? {} : ops.createObject();
