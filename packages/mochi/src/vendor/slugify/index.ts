...
// Vendored from https://github.com/simov/slugify 1.6.9
...
const charMap: Record<string, string> = JSON.parse(
...
const locales: Record<string, Record<string, string>> = JSON.parse(
...
function slugify(string: string, options?: Options | string): string {
  ...
  const opts: Options = typeof options === 'string' ? { replacement: options } : options || {};
  ...
  let slug = string
    ...
    .reduce((result, ch) => {
      ...
      return (
        ...
        appendChar.replace(opts.remove || /[^\w\s$*_+~.()'"!\-:@]+/g, '')
      );
    }, '');
...
slugify.extend = (customMap: Record<string, string>): void => {
