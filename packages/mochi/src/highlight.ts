 ...
 * button, and Svelte-brace escape.
 *
 * ```ts
 ...
 * ```
 */
export function createHighlighter(highlight: (code: string, lang: string) => string | Promise<string>): (code: string, lang?: string | null) => string | Promise<string> {
  return (code, lang) => {
    ...
    return result.then(wrapCodeBlock);
  };
}
