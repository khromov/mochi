...
describe('runMigrations (postgres)', () => {
  test('applies pending migrations in order, records them, and is idempotent', async () => {
    ...
    const applied = await runMigrations({ storage: { postgres: pg.url }, dir });
    ...
    expect(await runMigrations({ storage: { postgres: pg.url }, dir })).toEqual([]);
  });
  ...
  test('an edited applied migration is rejected as immutable', async () => {
    ...
    await runMigrations({ storage: { postgres: pg.url }, dir });
    writeFileSync(path.join(dir, '1_a.sql'), 'CREATE TABLE a (x integer, y integer);');
    await expect(runMigrations({ storage: { postgres: pg.url }, dir })).rejects.toThrow(/changed since it was applied.*immutable/);
  });
  ...
  test('a failing migration rolls back its statements and is not recorded; earlier ones stay', async () => {
    ...
    await expect(runMigrations({ storage: { postgres: pg.url }, dir })).rejects.toThrow(/nope/);
  ...
  test('framework and app tracking tables coexist in one database', async () => {
    ...
    await runMigrations({ storage: { postgres: pg.url }, dir, table: 'mochi_migrations', label: 'mochi' });
    await runMigrations({ storage: { postgres: pg.url }, dir: appDir, label: 'app' });
