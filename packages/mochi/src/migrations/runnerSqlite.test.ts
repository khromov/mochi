...
describe('runMigrations (sqlite)', () => {
  test('applies pending migrations in order, records them, and is idempotent', async () => {
    ...
    const applied = await runMigrations({ storage: { sqlite: db }, dir });
  ...
  test('an edited applied migration is rejected as immutable', async () => {
    ...
    await expect(runMigrations({ storage: { sqlite: db }, dir })).rejects.toThrow(/changed since it was applied.*immutable/);
  });
  ...
  test('fewer files than applied migrations is rejected', async () => {
    ...
    await expect(runMigrations({ storage: { sqlite: db }, dir })).rejects.toThrow(/2 applied migration\(s\) but only 1 file\(s\)/);
  });
  ...
  test('a failing migration rolls back its statements and is not recorded; earlier ones stay', async () => {
    ...
    await expect(runMigrations({ storage: { sqlite: db }, dir })).rejects.toThrow(/no such table/);
  ...
  test('an empty or missing dir touches nothing — not even the database file', async () => {
    expect(await runMigrations({ storage: { sqlite: db }, dir })).toEqual([]);
    expect(await runMigrations({ storage: { sqlite: db }, dir: path.join(dir, 'missing') })).toEqual([]);
    expect(existsSync(db)).toBe(false);
  });

  test('a custom tracking table is honored and validated', async () => {
    writeFileSync(path.join(dir, '1_a.sql'), 'CREATE TABLE a (x integer);');
    await runMigrations({ storage: { sqlite: db }, dir, table: 'mochi_migrations' });
    expect((await rows('SELECT id FROM mochi_migrations')).map((r) => r.id)).toEqual([1]);
    await expect(runMigrations({ storage: { sqlite: db }, dir, table: 'bad"name' })).rejects.toThrow(/not a valid table name/);
  });
});
