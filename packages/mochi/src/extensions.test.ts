...
describe('new extension points', () => {
  ...
  test('queue:maxTimeoutMs returns the default unchanged when no filter registered', () => {
    expect(applyFilter('queue:maxTimeoutMs', Infinity, { queue: 'emails', explicit: false })).toBe(Infinity);
  });

  test('queue:maxTimeoutMs can cap one queue only', () => {
    initExtensions({
      filters: {
        'queue:maxTimeoutMs': (def, { queue }) => (queue === 'transcode' ? 3_600_000 : def),
      },
    });
    expect(applyFilter('queue:maxTimeoutMs', Infinity, { queue: 'transcode', explicit: false })).toBe(3_600_000);
    expect(applyFilter('queue:maxTimeoutMs', Infinity, { queue: 'emails', explicit: false })).toBe(Infinity);
  });

  test('queue:maxTimeoutMs can leave queues that chose their own value alone', () => {
    initExtensions({
      filters: {
        'queue:maxTimeoutMs': (value, { explicit }) => (explicit ? value : 60_000),
      },
    });
    expect(applyFilter('queue:maxTimeoutMs', 50, { queue: 'transcode', explicit: true })).toBe(50);
    expect(applyFilter('queue:maxTimeoutMs', Infinity, { queue: 'emails', explicit: false })).toBe(60_000);
  });
