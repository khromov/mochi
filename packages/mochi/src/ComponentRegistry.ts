      ...
      setup(build) {
        ...
        build.onLoad({ filter: /.*/, namespace: 'mochi-env' }, () => ({
          contents: [
            ...
            `    return typeof r === "function" ? r.bind(v) : r;`,
            `  },`,
            `  has(_, p) { return p in new URL(window.location.href); },`,
            `  ownKeys() { return Reflect.ownKeys(new URL(window.location.href)); },`,
            `  getOwnPropertyDescriptor(_, p) {`,
            `    const d = Object.getOwnPropertyDescriptor(new URL(window.location.href), p);`,
            `    if (d) d.configurable = true;`,
            `    return d;`,
            `  },`,
            `});`,
