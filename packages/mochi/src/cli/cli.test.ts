...
describe('mochi-framework speculation-rules (subprocess)', () => {
  const ENTRY = `import { Mochi } from 'mochi-framework';

await Mochi.serve({
  routes: {
    '/': Mochi.page('Home.svelte'),
    '/blog/:slug': Mochi.page('Post.svelte'),
    '/api/x': Mochi.api(() => Response.json({})),
  },
});
`;

  async function entryIn(cwd: string) {
    const entry = path.join(cwd, 'index.ts');
    await Bun.write(entry, ENTRY);
    return entry;
  }

  it('lists speculation-rules in --help', async () => {
    const { exitCode, stdout } = await runCli('--help');

    expect(exitCode).toBe(0);
    expect(stdout).toContain('speculation-rules');
  });

  it('--dry-run prints prefetch + prerender derived from the routes', async () => {
    const entry = await entryIn(freshCwd());
    const { exitCode, stdout } = await runCli('speculation-rules', '--entry', entry, '--dry-run');
  ...
  it('writes the speculationRules key into the entry when not a dry run', async () => {
    const entry = await entryIn(freshCwd());
    const { exitCode, stdout } = await runCli('speculation-rules', '--entry', entry);
  ...
  it('exits non-zero when the entry is missing', async () => {
    const { exitCode, stderr } = await runCli('speculation-rules', '--entry', path.join(freshCwd(), 'nope.ts'));

    expect(exitCode).toBe(1);
    expect(stderr).toContain('Entry not found');
  });
});
