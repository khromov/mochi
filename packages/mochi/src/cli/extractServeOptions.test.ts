...
throw new Error('serve should have halted execution before this line');`,
  ...
  test('captures the queues map without starting a live queue', async () => {
    // Mochi.queue() is inert config, so importing the entry for extraction must
    // not construct a live better-queue instance (whose store handle and timers
    // would hang the build). The test simply completing proves nothing kept the
    // event loop alive.
    const entry = writeEntry(
      `import { Mochi } from 'mochi-framework';
await Mochi.serve({ routes: {}, queues: { emails: Mochi.queue({ process: async () => ({ sent: true }), concurrent: 2 }) } });
throw new Error('serve should have halted execution before this line');`,
    );
    ...
    expect(options?.queues?.emails?.options).toEqual({ concurrent: 2 });
  });
