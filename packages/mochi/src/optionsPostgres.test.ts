...
describe('MochiOptions on postgres storage', () => {
  ...
  test('installs its schema and roundtrips get/set/update/delete', async () => {
    ...
    // The zero-row RETURNING behind this rejection is what wedges a cached prepared statement on Bun 1.3.x
    // for Windows, so it doubles as the regression guard for the driver's `prepare: false`.
