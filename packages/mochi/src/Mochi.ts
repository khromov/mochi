...
export class Mochi {
  ...
  static async serve(options: MochiServeOptions): Promise<Server<undefined>> {
    ...
    // A no-op with the debug bar off, awaited before `Bun.serve()` so its script exists and its URL is settled before the
    // first request. Kept out of the per-page client bundle so on-demand recompiles never re-hash it.
