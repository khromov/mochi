...
describe('mochiEvents', () => {
  ...
  test('queue:completed is part of the event map', () => {
    let received: { queue: string; jobId: string; attempt: number } | null = null;
    const handler = (e: { queue: string; jobId: string; attempt: number; duration: number }) => {
      received = { queue: e.queue, jobId: e.jobId, attempt: e.attempt };
    };
    ...
    expect(received as { queue: string; jobId: string; attempt: number } | null).toEqual({
      queue: 'emails',
      jobId: 'j1',
      attempt: 1,
    });
