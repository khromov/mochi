...
beforeAll(async () => {
  ...
  await Bun.write(
    path.join(app, 'value.compiled.ts'),
...
describe('a *.compiled.ts module through a real compile', () => {
  test('bakes the exports into the SSR chunk and leaves the producing module out', async () => {
    ...
    expect(registry.getBuildTimeModules()).toEqual([path.join(app, 'value.compiled.ts')]);
  }, 60_000);
  ...
  test('an evaluation error surfaces as a compile error naming the module', async () => {
    await Bun.write(path.join(app, 'broken.compiled.ts'), `export const fn = () => 1;\n`);
    await Bun.write(path.join(app, 'Broken.svelte'), `<script>\n  import { fn } from './broken.compiled.ts';\n</${'script'}>\n<p>{fn}</p>\n`);
    const registry = new ComponentRegistry({ development: false, outDir });
    const failure = await registry.compileAll([path.join(app, 'Broken.svelte')]).then(
      () => '',
      (e: Error) => e.message,
    );
    expect(failure).toContain('broken.compiled.ts exports "fn", which cannot be inlined');
