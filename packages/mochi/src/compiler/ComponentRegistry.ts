      ...
      setup(build) {
        ...
        build.onLoad({ filter: /.*/, namespace: 'mochi-env' }, () => ({
          contents: [
            ...
            `import { logger as __mochi_logger, setLogLevel, getLogLevel } from "${toPosixPath(path.join(SRC_DIR, 'utils/log.ts'))}";`,
            ...
            `export { stringify, parse } from "${toPosixPath(Bun.resolveSync('devalue', SRC_DIR))}";`,
            `export { trailingSlashIt } from "${toPosixPath(path.join(SRC_DIR, 'runtime/trailingSlash.ts'))}";`,
            // Per-request hydratable-island props dedup helper. Used by the
            // preprocessor's injected `__mochi_emit_props__` import.
            `export { emitIslandProps } from "${toPosixPath(path.join(SRC_DIR, 'islands/islandPropsRegistry.ts'))}";`,
            ...
            `import __mochi_mitt__ from "${toPosixPath(Bun.resolveSync('mitt', SRC_DIR))}";`,
            ...
            `export { enhance, deserialize } from "${toPosixPath(path.join(SRC_DIR, 'runtime/enhance.ssr.ts'))}";`,
            // Rate-limit stores — server-only (bun:sqlite / Bun SQL).
            `export { memoryStore, sqliteStore, postgresStore } from "${toPosixPath(path.join(SRC_DIR, 'runtime/rateLimit.ts'))}";`,
          ].join('\n'),
          loader: 'js',
        }));
