...
export class ComponentRegistry {
  ...
  private createClientBuildPlugin(locals: {
    ...
    const shakenSources = this.shakenSources;
    const compileCache = this.compileCache;
    const { compileCacheStats, clientFingerprint, backend, debugBarDir, cookiesClientPath, enhanceClientPath } = locals;
    const imageAssetLoader = createImageAssetLoader({
      outDir: this.outDir,
      assetPrefix: this.assetPrefix,
      assets: this.localImageAssets,
  ...
  private async buildClientBundle(): Promise<void> {
    ...
    const clientPlugin = this.createClientBuildPlugin({ compileCacheStats, clientFingerprint, backend, debugBarDir, cookiesClientPath, enhanceClientPath });
  ...
  /**
   * Builds the dev debug bar once, as its own self-contained (`splitting: false`) bundle under a stable `/debugbar/` URL
   * prefix. Deliberately kept out of `buildClientBundle`: on-demand page / server-island compiles re-run that build,
   * which deletes and re-hashes every `/client/` asset, 404-ing the `<script src>` a page already in the browser pins.
   * The debug-bar code is runtime-invariant, so one build under a prefix the per-page rebuild never touches keeps that
   * URL alive for the whole process. Idempotent and self-healing: a no-op once built, rebuilt if the file was cleared.
   */
  async buildDebugBar(): Promise<void> {
    ...
    const compileCacheStats = createCompileCacheStats();
    const backend = await resolveSvelteCompiler(this.svelteCompiler);
    const clientFingerprint = compileFingerprint(this.svelteConfig.compilerOptions ?? {}, this.development, backendId(backend));
    const plugin = this.createClientBuildPlugin({ compileCacheStats, clientFingerprint, backend, debugBarDir, cookiesClientPath, enhanceClientPath });
    ...
    try {
      const result = await Bun.build({
        ...
        // Same ordering rule as the per-page client build: the guard resolves
        // server-only specifiers before the client plugin can claim them.
        plugins: [serverOnlyModuleGuard, plugin],
        target: 'browser',
