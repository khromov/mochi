...
beforeAll(async () => {
  outDir = mkdtempSync(path.join(import.meta.dir, '..', '..', '.mochi-compiled-moduleref-'));
  app = path.join(outDir, 'app');
  await Bun.write(path.join(app, 'Card.svelte'), `<p class="card">card markup</p>\n`);
  await Bun.write(
    path.join(app, 'cards.compiled.ts'),
...
describe('moduleRef() in a *.compiled.ts module', () => {
