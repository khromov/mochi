...
import { createModuleRef, isModuleRef, serializePrerenderValue } from './prerenderSerialize';

describe('serializePrerenderValue', () => {
  test('round-trips rich types devalue supports but JSON does not', () => {
    ...
    const { expression, imports } = serializePrerenderValue(value);
  ...
  test('preserves cycles and shared references', () => {
    ...
    value.self = value;
    const revived = new Function(`return ${serializePrerenderValue(value).expression};`)() as Record<string, unknown>;
  ...
  test('escapes markup that would otherwise close the surrounding script tag', () => {
    ...
    const { expression } = serializePrerenderValue({ html: payload });
  ...
  test('recognises markers across framework copies via a registry symbol', () => {
    // A `*.prerender.ts` module may hold a different `mochi-framework` instance than the compiler does, so the brand
