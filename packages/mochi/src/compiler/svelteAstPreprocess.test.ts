...
describe('preprocessHydratable - <Script>', () => {
  ...
  test('the entry key hashes the encoded source path, so it is machine-independent', () => {
    const source = `${SCRIPT(IMPORT)}<Script src="./a.ts" />`;
    const { scriptEntries } = preprocessHydratable(source, filePath);
    expect(scriptEntries[0]!.key).toBe(Bun.hash(encodeSourcePath(path.join(dir, 'a.ts'))).toString(36));
  });
  ...
  test('reports unresolvable paths with forward slashes', () => {
    const source = `${SCRIPT(IMPORT)}<Script src="./missing.ts" />`;
    try {
      preprocessHydratable(source, filePath);
      expect.unreachable();
    } catch (e) {
  ...
  test('a hydrate directive on <Script> reports the server-only island error instead of rewriting', () => {
    const source = `${SCRIPT(IMPORT)}<Script src="./a.ts" mochi:hydrate />`;
    const { scriptEntries, errors, transformed } = preprocessHydratable(source, filePath);
    expect(scriptEntries).toHaveLength(0);
    expect(errors).toHaveLength(1);
    expect(errors[0]!.reason).toBe('server-only');
    expect(transformed).not.toContain('__mochiScriptUrls');
  });
});
