...
describe('server island endpoint', () => {
  ...
  beforeAll(async () => {
    ...
    const decoded = decryptProps(token, 'Echo');
    if (!decoded) {
  ...
  test('islandId containing `--` skips namespacing instead of failing the render', async () => {
    const legacy = encryptProps(devalueStringify({ islandId: 'mochi--legacy-0', name: 'Legacy' }), 'Echo');
  ...
  test('missing islandId renders un-namespaced rather than failing', async () => {
    const noId = encryptProps(devalueStringify({ name: 'Bare' }), 'Echo');
  ...
  test('injects <link> tags for CSS the host page did not link', async () => {
    const props = encryptProps(devalueStringify({ islandId: 's-css-0' }), 'StyledLeaf');
