...
describe('Mochi.file', () => {
  ...
  test('emits a request event with kind "asset" and an error event with kind "file"', async () => {
    ...
    expect(requests.some((e) => e.kind === 'asset' && e.path === '/files/report' && e.status === 200)).toBe(true);
