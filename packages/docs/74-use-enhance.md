...
When the same component renders both as a hydrated island (where `enhance` will fire) and as a plain SSR-only child (where it won't), call [`isHydratable()`](/docs/environment-constants/#ishydratable) to skip the SSR `form`-prop peek when the client will take over.
