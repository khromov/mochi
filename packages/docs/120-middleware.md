...
Static serving flows through `handle`, so `compress()` covers it: framework bundles, extracted fonts, imported CSS and locally-imported assets (`event.kind === 'asset'`, including `/_mochi/island/*` server-island fragments), `publicDir` files (`event.kind === 'public'`), and `Mochi.file()` routes (`event.kind === 'file'`) — on `HEAD` as well as `GET`. The one exception is the generated-image endpoint (`/_mochi/image/*`), which stays on a native route: it emits webp/avif, which `compress()` skips anyway.

Files at stable URLs (`public`/`Mochi.file()`) keep their `ETag`/`Last-Modified` conditional-request and byte-range support. Body-touching middleware that needs to skip static responses should branch on `event.kind` — e.g. skip the framework/static kinds `asset`, `public`, and `file`. `transformPage` already skips `public` and `file` for you, and marks island fragments with `kind: 'deferredIsland'`; see [Transforming HTML](/docs/transform-page/).
