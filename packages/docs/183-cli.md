...
title: 'CLI reference'
slug: cli
description: 'The mochi-framework command-line tool: build, migrate, generate-key, and update-skill.'
---
...
<VersionNote since="0.10.0" message="The migrate command ships in 0.10.0." />

Applies pending SQL migrations from `migrations/<sqlite|postgres>/` without booting the server, reading `storage` from the entry's `Mochi.serve()` call:

```sh
bunx mochi-framework migrate
```
...
See [Migrations](/docs/migrations/) for the file format and rules.
