...
title: 'CLI reference'
slug: cli
description: 'The mochi-framework command-line tool: build, generate-key, speculation-rules, and update-skill.'
---
...
Reads your entry's `Mochi.serve({ routes })` call and writes a starting [Speculation Rules](/docs/speculation-rules/) config into its `speculationRules` key:

```sh
bunx mochi-framework speculation-rules
```

| Option           | Default          | Description                                               |
| ---------------- | ---------------- | --------------------------------------------------------- |
| `--entry <path>` | `./src/index.ts` | Runtime entry whose `Mochi.serve()` call supplies routes. |
| `--dry-run`      |                  | Print the generated rules instead of writing them.        |

Dynamic `:param` route segments become `*` globs, and prerender is limited to your static pages. Treat the output as a starting point, then run your formatter to tidy it.
