...
Every SSR entrypoint it compiled is listed as one tree. The routes come first, then two groups that no URL reaches — the [error page](/docs/error-handling/), which renders on a throw, and your [email templates](/docs/email/#svelte-templates), which render on a send:
...
```
  ...
  │
  │   Error page
  ├ ⚠ $mochi/templates/DefaultError.svelte          1  91.4 kB
  │
  │   Email template
  ├ ✉ src/emails/Receipt.svelte                     -    316 B
  └ ✉ src/emails/Welcome.svelte                     -    288 B

  ○ ssr-only page  ·  λ api  ·  ⚠ error page  ·  ✉ email template
```

The error-page row is Mochi's built-in one until you set [`errorPage`](/docs/error-handling/#errorpage), which is why it reads `$mochi/…` above. Email templates are found by walking `src/emails/` — nothing imports one from a route, so there is no import graph leading there — and their islands column reads `-` because an island in a template [fails the build](/docs/email/#svelte-templates).
