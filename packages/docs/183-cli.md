...
The manifest stores all artifact paths relative to the out-dir, so the build output is relocatable — move or copy it to another directory or machine and boot with `outDir` (or `manifest`) pointing at the new location. See [Relocatable builds](/docs/deployment-options/) for the remaining constraints and how the output is served.
