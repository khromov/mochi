...
- `build`: Output controls for `mochi-framework build`; ignored by the runtime. `showImages`, `showStyles`, `showScripts` and `showStaticFiles` (all default `true`) each hide one section of the build's resource report; the summary line keeps its counts regardless. See `CLI`.
