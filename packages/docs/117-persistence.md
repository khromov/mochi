...
- **[Rate limiting](/docs/rate-limiting/)** — `rateLimit: { store }`. `rateLimitMemoryStore()` (default), `rateLimitSqliteStore({ path })` and `rateLimitPostgresStore({ url })` all ship with the framework; `MochiRateLimitStore` is the interface for your own. Create the store once and share the instance across routes.
