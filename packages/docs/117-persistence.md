...
- **[Migrations](/docs/migrations/)** — the serve-level `storage` (`{ type: 'sqlite', path }` or `{ type: 'postgres', url }`) names the app database; forward-only `.sql` files from `migrations/<sqlite|postgres>/` are applied against it on startup. No in-memory mode — migrations only make sense against something durable.
