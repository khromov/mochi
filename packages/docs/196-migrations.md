...
One block per version, listing every breaking change and the lever that addresses it. Where the old behaviour can be preserved, the option that does it is named.
...
- Mochi requires Bun 1.4 or newer. Upgrade Bun; the version gate runs before the server binds and has no opt-out.
...
- `HttpOnly` also stops the browser overwriting or deleting a cookie, so client-side writes to a server-set name are silently ignored. Mochi logs a warning when this happens; the same `{ httpOnly: false }` resolves it.
- Cross-origin WebSocket upgrades are rejected in production. Add the origins to `csrf.trustedOrigins`, or set `csrf.checkOrigin: false` to keep the old behaviour — that switch also disables the form CSRF check.
...
- `redirect()` rejects any location containing control characters, including tab and DEL. Encode the value before passing it; this cannot be waived.
- `redirect()` treats `mailto:`, `tel:` and other non-`http(s)` schemes as off-origin. Pass `{ external: true }` on those calls.
- With no `proxy.origin` or `proxy.hostHeader` set, production blocks every absolute redirect location. Configure `proxy.origin`, which the CSRF check needs anyway.
...
- `Mochi.queue()` takes the queue name as its first argument, and `queues` is an array of descriptors rather than an object. Update the call sites; there is no opt-out.
...
- A queue whose declared config differs from the stored one throws at boot. Set `queueConfig: 'sync'` to adopt the declared config instead.
...
- The `queue:*` events no longer carry `jobName`. Read the queue name from the event instead.
...
- `fail()` or `success()` returned from `serverProps` now throws. Return props or a redirect from `serverProps` instead.
...
- A `.server.svelte` component cannot be hydrated, since the suffix now marks a component the client build stubs out. Rename the file if the suffix was incidental.
...
- `compress()` dropped brotli and defaults to zstd, so responses go out with `Content-Encoding: zstd`. Set `methods: ['gzip']` if a CDN, proxy or WAF in front of the app cannot handle zstd.
...
- `@font-face` rules are stripped from rendered emails, which clients ignored or clipped anyway. Use web-safe fonts in email templates; there is no opt-out.
...
- `svelte-shaker` is now an optional peer dependency, so `optimize` builds unshaken without it and only warns. Install `@mochi-framework/svelte-shaker`.
