...
- **[`compress()`](/docs/middleware/#compress) streams**, except for brotli, which buffers the whole body before compressing it. If a streamed response only arrives once the handler finishes, check whether the client negotiated `br` — `compress({ methods: ['zstd', 'gzip'] })` keeps every path streaming.
